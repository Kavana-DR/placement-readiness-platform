...
export default function RecommendedInternships() {
  ...
  const recommendations = React.useMemo(
    () => getRecommendedInternships(internshipsData, resumeSkills),
    [resumeSkills],
  )
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Matched: {job.matchedSkills.length > 0 ? job.matchedSkills.join(', ') : 'No direct skill match'}
                    </p>
                  </div>
                  <button
                    type="button"
                    className={`px-3 py-1 rounded-md text-sm ${
                      appliedJobKeys.has(`${job.title}::${job.company}`)
                        ? 'bg-gray-200 text-gray-700 cursor-not-allowed'
                        : 'bg-primary text-white'
                    }`}
                    disabled={appliedJobKeys.has(`${job.title}::${job.company}`)}
                    onClick={() => handleApply(job)}
                  >
                    {appliedJobKeys.has(`${job.title}::${job.company}`) ? 'Applied' : 'Apply'}
                  </button>
                </div>
              </li>
            ))}
